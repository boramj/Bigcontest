...
library(DMwR) ; library(xgboost);library(e1071)
...
#데이터 나누기(8:2)
set.seed(1)
trainIndex <- createDataPartition(data_set$TARGET, p = .95)
...
confusionMatrix(xgb.pred1,data_age1te$TARGET)
confusionMatrix(xgb.pred2,data_age2te$TARGET)
confusionMatrix(xgb.pred3,data_age3te$TARGET)
