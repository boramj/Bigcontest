...
#TARGET을 numeric으로 다시 바꿔주자
# data_set$TARGET<-ifelse(data_set$TARGET=='0',0,1)
# dmy <- dummyVars( ~ ., data = data_set,fullRank = T)
# data_transformed <- data.frame(predict(dmy, newdata = data_set))
# str(data_transformed)

# #변수선택하기
# data_set_feature<-data_set[,-c()]
# str(data_set_zero)
...
################################
set.seed(1)
trainIndex <- createDataPartition(data_set$TARGET, p = .8, list=F)
...
set.seed(1)
#변수선택 
smote_train <- smote_train[,-c(16,21,22,23,25,59,61,66)]
