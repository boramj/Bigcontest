...
####Age Clustering
data_age1tr<-subset(nodatatrain, AGE==20 |AGE==25|AGE==30|AGE==35)
data_age2tr<-subset(nodatatrain, AGE==40 |AGE==45|AGE==50|AGE==55)
data_age3tr<-subset(nodatatrain, AGE==60 |AGE==65|AGE==70|AGE==0)

table(data_age1tr$TARGET)
table(data_age2tr$TARGET)
table(data_age3tr$TARGET)

data_age1te<-subset(nodatatest, AGE==20 |AGE==25|AGE==30|AGE==35)
data_age2te<-subset(nodatatest, AGE==40 |AGE==45|AGE==50|AGE==55)
data_age3te<-subset(nodatatest, AGE==60 |AGE==65|AGE==70|AGE==30)

table(data_age1te$TARGET)
table(data_age2te$TARGET)
table(data_age3te$TARGET)


#불균형 맞추기
set.seed(1)
smote_age1 <- SMOTE(TARGET ~ ., data_age1tr)
smote_age2 <- SMOTE(TARGET ~ ., data_age2tr)
smote_age3 <- SMOTE(TARGET ~ ., data_age3tr)
table(smote_age1$TARGET)
table(smote_age2$TARGET)
table(smote_age3$TARGET)
...
set.seed(1)
random1 = randomForest(TARGET~.,data=smote_age1, mtry = 20,
                      importance =T)
random2 = randomForest(TARGET~.,data=smote_age2, mtry = 20,
                       importance =T)
random3 = randomForest(TARGET~.,data=smote_age3, mtry = 20,
                       importance =T)

yrandom1 = predict(random1, newdata=data_age1te)
yrandom2 = predict(random2, newdata=data_age2te)
yrandom3 = predict(random3, newdata=data_age3te)

result1 <- confusionMatrix(yrandom1, data_age1te$TARGET)
result2 <- confusionMatrix(yrandom2, data_age2te$TARGET)
result3 <- confusionMatrix(yrandom3, data_age3te$TARGET)
