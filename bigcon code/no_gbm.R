...
library(rpart) ;library(rpart.plot) ;library(randomForest) ;library(caret) ;
library(DMwR)
...
######xgboost##############
#xgboost는 target에 0,1이 들어가있으면 돌아가지 않는다. 바꿔주자
...
traincr <- trainControl(method = "repeatedcv",   # 10fold cross validation
                     number = 5,	
                     classProbs = TRUE,# do 5 repititions of cv
...
gbm.tune <- train(TARGET~., data = smote_train,
                  method='gbm',
                  metric='ROC',
                  trControl = traincr,
                  tuneGrid=grid,
...
##
gbm.pred <- predict(gbm.tune,nodatatest)
confusionMatrix(gbm.pred,nodatatest$TARGET)
