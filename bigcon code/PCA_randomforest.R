...
library(DMwR) ; library(xgboost);library(e1071);library(psych)
...
################################################
str(data_set)
data_set_pca<-data_set[,-c(1,16,21,22,25,53,56,66)]
pr.out = prcomp(data_set_pca,scale = TRUE)

#par(mfrow = c(1,1))
#biplot(pr.out, scale = 0)

#############################################
smote_data_pca<-data.frame(TARGET = data_set$TARGET,AGE=data_set$AGE,SEX=data_set$SEX,pr.out$x)
str(smote_data_pca)
##############################
##########모델링##############
##############################
#데이터 나누기(9.5:0.5)
set.seed(1)
trainIndex <- createDataPartition(smote_data_pca$TARGET, p = .95)
dataTrain <- smote_data_pca[trainIndex$Resample1,]
dataTest  <- smote_data_pca[-trainIndex$Resample1,]
str(dataTrain)
str(dataTest)

#불균형 맞추기
set.seed(1)
smote_data <- SMOTE(TARGET ~ ., dataTrain, perc.over = 1000)
...
set.seed(1)
random = randomForest(TARGET ~ ., data=smote_data,importance =T)
yrandom1 = predict(random, newdata=dataTest)
varImpPlot(random)
